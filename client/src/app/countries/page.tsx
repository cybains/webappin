...
export default function CountriesPage() {
  ...
  const totalCount = useMemo(() => GROUPS.reduce((acc, g) => acc + g.countries.length, 0), []);
  ...
  return (
    <>
      <main className="max-w-6xl mx-auto p-6 space-y-10 min-h-screen bg-[var(--background)] text-[var(--foreground)]">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end gap-4">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs px-2 py-1 rounded-full chip">{shownCount}/{totalCount}</span>
              ...
              <input
                ...
                className="w-72 max-w-full rounded-xl px-3 py-2 text-sm focus:outline-none"
                style={{
                  background: 'var(--card)',
                  border: '1px solid var(--border)',
                  color: 'var(--foreground)',
                  boxShadow: '0 0 0 1px color-mix(in srgb, var(--ring) 30%, transparent)',
                }}
                autoComplete="off"
              />
            </div>
        ...
        {/* Groups */}
        {filtered.length === 0 ? (
          <div className="p-6 card rounded-2xl ring-brand">No countries match “{q}”. Try a different name or code.</div>
        ) : (
          filtered.map((group) => {
            ...
            return (
              <section key={group.id} className="space-y-3">
                ...
                >
                  ...
                  <span className={['ml-auto text-xs px-2 py-0.5 rounded-full chip'].join(' ')}>
                    ...
                    {group.countries.map((c) => {
                      ...
                      return (
                        <Link
                          ...
                          href={`/country/${c.code}`}
                          className={[
                            'group rounded-2xl p-4 transition focus:outline-none',
                            'card ring-brand shadow-sm hover:-translate-y-0.5 hover:shadow-md',
                          ].join(' ')}
